...
function controlInfo(obj, boolean) {
	if (obj && obj.insumos.id_proveedor != null) {
		...
		unidadInfo.value = obj.insumos.unidad;
		cantidadInfo.value = obj.insumos.cantidad;
		totalInfo.value = obj.insumos.total;
	}
	if (obj && obj.semillas.id_proveedor != null) {
		mostrarInfo.value = "Semilla";
		proveedorGastos.value = obj.insumos.id_proveedor.nombre;
		semillaInfo.value = obj.semillas.id_semillas;
		unidadInfo.value = obj.semillas.unidad;
		cantidadInfo.value = obj.semillas.cantidad;
		totalInfo.value = obj.semillas.total;
	}
}
...
<template>
	<div>
				...
				<template v-slot:body-cell-informacion="props">
					<q-td :props="props">
						<q-btn
							...
							" @click="controlInfo(props.row, true)">
							👁️
						</q-btn>
		...
		<q-dialog v-model="mostrarInformacion">
			<q-card>
				<p>
					proveedor<span>{{ proveedorInfo }}</span>
				</p>
				<p v-if="mostrarInfo == 'Insumo'">
					insumo<span>{{ insumoInfo }}</span>
				</p>
				<p v-if="mostrarInfo == 'Semilla'">
					semilla<span>{{ semillaInfo }}</span>
				</p>
				<p>
					unidad<span>{{ unidadInfo }}</span>
				</p>
				<p>
					total<span>{{ totalInfo }}</span>
				</p>
				<p>
					cantidad<span>{{ cantidadInfo }}</span>
				</p>
			</q-card>
		</q-dialog>
