...
const addPreparacionSuelo = async () => {
	try {
		...
		await usePS.postPreparacionSuelo(
...
const updatePreparacionSuelo = async () => {
	try {
		...
		await usePS.putPreparacionSuelo(
			datos.value._id,
			preparacionSueloData
		);
...
const changePreparacionSueloState = async (id, estado) => {
	try {
		...
		console.error(
			"Error al cambiar el estado de la preparación de suelo:",
			error
		);
	}
};
...
function administrarFormulario(datos, boolean) {
	...
	if (datos != null) {
		objPreparacionSuelo.value = { ...datos };
	...
	<div>
		...
		<q-table
			...
			<template v-slot:top>
				<div class="q-mb-md">
					<q-select
						v-if="opcion.value === 'Parcelas'"
						:options="opcionesParcelas"
						v-model="parcela"
						label="Selecciona Parcela" />
					<q-input
						v-if="opcion.value === 'Fechas'"
						v-model="fechaInicio"
						label="Fecha Inicio"
						type="date" />
					<q-input
				...
				<q-select
					...
					label="Opciones Tabla" />
			</template>
			<template v-slot:body-cell-actions="props">
				<q-btn
					icon="edit"
					color="primary"
					@click="administrarFormulario(props.row, true)" />
				<q-btn
					icon="delete"
					color="negative"
					@click="
						changePreparacionSueloState(
							props.row._id,
							props.row.estado
						)
					" />
		...
		<q-dialog v-model="mostrarFormularioPrepararSuelo">
			<q-card>
				<q-form>
					<q-input
						v-model="objPreparacionSuelo.fecha"
						label="Fecha"
						type="date" />
					<q-select
						:options="opcionesParcelas"
						v-model="objPreparacionSuelo.id_parcela"
						label="Parcela" />
					<q-select
						:options="opcionesEmpleado"
						v-model="objPreparacionSuelo.empleado_id"
						label="Empleado" />
					<q-input
						v-model="objPreparacionSuelo.productos"
						label="Productos" />
					<q-input
						v-model="objPreparacionSuelo.ingredienteActivo"
						label="Ingrediente Activo" />
					<q-input
						v-model="objPreparacionSuelo.dosis"
						label="Dosis" />
					<q-input
						v-model="objPreparacionSuelo.metodoAplicacion"
						label="Método de Aplicación" />
					<q-input
						v-model="objPreparacionSuelo.responsable"
						label="Responsable" />
					<q-input
						v-model="objPreparacionSuelo.observaciones"
						label="Observaciones" />
					<div>
						<q-btn
							@click="administrarFormulario(null, false)"
							label="Cancelar" />
						<q-btn
							v-if="!mostrarBotonEditar"
							@click="addPreparacionSuelo()"
							color="primary"
							label="Agregar" />
						<q-btn
							v-else
							@click="updatePreparacionSuelo()"
							color="primary"
							label="Editar" />
					</div>
				</q-form>
