...
import { QSelect, QInput, QBtn, QDialog, QCard, QCardSection, QForm, QTable, QTd, QTooltip, QTr } from "quasar";
...
const columns = ref([
  ...
  { name: "empleado_id", label: "Empleado ID", field: "empleado_id", align: "center" },
  { name: "fecha", label: "Fecha", align: "center", field: (row) => format(new Date(row.fecha), "dd/MM/yyyy") },
  { name: "tipoClima", label: "Tipo de Clima", field: "tipoClima", align: "center" },
  { name: "horaInicio", label: "Hora de Inicio", field: "horaInicio", align: "center" },
  { name: "horaFinal", label: "Hora Final", field: "horaFinal", align: "center" },
  { name: "temperaturaMaxima", label: "Temperatura Máxima", field: "temperaturaMaxima", align: "center" },
  { name: "temperaturaMinima", label: "Temperatura Mínima", field: "temperaturaMinima", align: "center" },
...
async function editarClima() {
  ...
  try {
    const response = await useClima.putClima(idClimaSeleccionado.value, climaActualizado);
    if (response.status === 200) {
      listarClimas();
...
const filteredRows = computed(() => {
  if (selectedOption.value === "fecha" && fechaSeleccionada.value) {
    return rows.value.filter((row) => row.fecha === fechaSeleccionada.value);
  } else if (selectedOption.value === "tipoClima" && tipoClimaSeleccionado.value) {
    return rows.value.filter((row) => row.tipoClima.toLowerCase().includes(tipoClimaSeleccionado.value.toLowerCase()));
  }
  return rows.value;
});
  ...
  <div class="q-pa-md">
    <div>
      ...
      <div style="margin-left: 5%; text-align: end; margin-right: 5%" class="q-my-md">
        <q-btn label="Agregar Clima" @click="mostrarFormularioAgregarClima = true">
          <q-tooltip>
            Agregar Clima
          </q-tooltip>
        </q-btn>
        ...
        <q-card>
          ...
          <q-card-section>
            <q-form @submit.prevent="agregarClima">
              <q-input v-model="finca_id" label="ID de Finca" filled required class="q-mb-md" />
              <q-input v-model="empleado_id" label="ID de Empleado" filled required class="q-mb-md" />
              <q-input v-model="fecha" label="Fecha" type="date" filled required class="q-mb-md" />
              <q-input v-model="tipoClima" label="Tipo de Clima" filled required class="q-mb-md" />
              <q-input v-model="horaInicio" label="Hora de Inicio" filled required class="q-mb-md" />
              <q-input v-model="horaFinal" label="Hora de Finalización" filled required class="q-mb-md" />
              <q-input v-model="temperaturaMaxima" label="Temperatura Máxima" type="number" filled required class="q-mb-md" />
              <q-input v-model="temperaturaMinima" label="Temperatura Mínima" type="number" filled required class="q-mb-md" />

              <div class="q-gutter-md row justify-center">
                <q-btn label="Agregar" type="submit" color="primary" />
                <q-btn label="Cancelar" color="negative" @click="cancelarClima" />
              </div>
            </q-form>
          </q-card-section>
        </q-card>
